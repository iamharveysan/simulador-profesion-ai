...
const EVENT_NAME       = process.env.EVENT_NAME || "USB con La Roja 🇪🇸";
const AUTO_RESET_DAILY = String(process.env.AUTO_RESET_DAILY || "false").toLowerCase() === "true";
...
/**
 * Multer: acepta la foto principal + hasta 3 imágenes de referencia
 * Campos: photo (1), ref_image_0, ref_image_1, ref_image_2
 */
const upload = multer({
  ...
  { name: "ref_image_0",maxCount: 1 },
  { name: "ref_image_1",maxCount: 1 },
  { name: "ref_image_2",maxCount: 1 },
]);
...
async function pruneGallery(maxFiles = MAX_GALLERY_FILES) {
  try {
    ...
    await Promise.all(
      detailed.slice(maxFiles).map(f => fsp.unlink(f.full).catch(() => {}))
    );
  } catch (err) {
...
/* Generación principal — soporta foto + imágenes de referencia */
app.post("/render-edit", (req, res, next) => {
  upload(req, res, err => {
  ...
  try {
    const userPrompt = (req.body?.prompt || "Generate a creative composite portrait").trim();

    const photoFile = req.files?.["photo"]?.[0];
    if (!photoFile?.buffer) {
      return res.status(400).json({ error: "Falta la foto principal." });
    }

    /* Construir parts para Gemini:
       1. Texto con el prompt
       2. Foto del usuario
       3. Imágenes de referencia (0, 1, 2) si existen
    */
    const parts = [
      {
        text:
          `${userPrompt}. ` +
          `IMPORTANT: Preserve the real person's authentic facial features, identity, and appearance completely. ` +
          `Return only one final composite image without captions, text overlays, or extra elements.`
      },
      {
        inlineData: {
    ...
    // Agregar imágenes de referencia si se enviaron
    for (let i = 0; i <= 2; i++) {
      const refFile = req.files?.[`ref_image_${i}`]?.[0];
      if (refFile?.buffer) {
        parts.push({
          inlineData: {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: data?.error?.message || data?.raw || "Google Images API error",
      });
    }
    ...
    if (!imagePart) {
      const hint = parts2.find(p => typeof p?.text === "string")?.text;
    ...
    res.status(500).json({ error: "Fallo al generar imagen con Google API." });
  }
});
...
app.post("/api/save-generation", async (req, res) => {
  try {
    ...
    const { image_base64, program_key, program_name, client_id, phrase } = req.body || {};

    if (!image_base64 || !program_key || !program_name || !client_id) {
      return res.status(400).json({ error: "Faltan datos para guardar la participación." });
    }

    const cleanKey = safeProgramKey(program_key);
    const now = new Date().toISOString();

    if (!stats.clients[client_id]) {
      stats.clients[client_id] = { first_seen: now, last_seen: now, generations: 0 };
    ...
    if (!stats.program_counts[cleanKey]) {
      stats.program_counts[cleanKey] = { name: program_name, count: 0 };
    }
    stats.program_counts[cleanKey].name  = program_name;
    stats.program_counts[cleanKey].count += 1;

    const filename = `${Date.now()}_${cleanKey}.png`;
    ...
    res.status(500).json({ error: "No fue posible registrar la participación." });
  }
});
...
app.post("/api/admin/reset-event", async (req, res) => {
  try {
    ...
    if (!ADMIN_RESET_TOKEN) return res.status(400).json({ error: "No configuraste ADMIN_RESET_TOKEN en .env" });
    if (token !== ADMIN_RESET_TOKEN) return res.status(401).json({ error: "Token inválido." });

    const newEventName = String(req.body?.event_name || EVENT_NAME).trim() || EVENT_NAME;
    const files = await fsp.readdir(GALLERY_DIR).catch(() => []);
    await Promise.all(files.map(name => fsp.unlink(path.join(GALLERY_DIR, name)).catch(() => {})));

    const newStats = initialStats(newEventName);
    await writeStats(newStats);
    res.json({ ok: true, stats: publicStats(newStats) });
  } catch (err) {
    console.error("❌ Reset event:", err);
    res.status(500).json({ error: "No fue posible reiniciar la jornada." });
  }
});
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`✅ USB · La Roja server on http://0.0.0.0:${PORT}`);
